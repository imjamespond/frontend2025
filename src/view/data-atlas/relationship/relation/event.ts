...
export function handleNodeContextMenu(this: Graph) {
  this.graph.on("node:contextmenu", ({ node }) => {
    ...
    n.selected = true;
    node.attr("g/class", "__contextmenu");
    const selection = select(container).select<SVGGElement>(".__menu").datum(n);
...
// click blank
export function handleBlankClick(this: Graph) {
  this.graph.on("blank:click", () => {
    this.deSelectNode();
  });
}

export function deSelectNode(this: Graph) {
